...
K <- 4 # Number of harder post-class quizzes
...
**Make sure you attend the correct section. This is section 501. You will not receive credit for assignments and/or exams taken in another section.**

There is no credit for attendance in this class, nor are there any points for any in-class activities. That said, my lecture slides will only give you an outline for the course and will not serve as a substitute for attendance.
...
The last `r K` (Chapters 21-23),$^{\dagger\dagger\dagger}$ however, are conceptually a bit harder and are worth `r J`. Together, these sum to `r (H*I)+(J*K)` points, or `r paste0(round((sum((H*I),(J*K))/N)*100,0),"%")` of your total grade).
...
pack_rows("Preliminaries", 1, 1, background = "#D6D3C4") %>%
pack_rows("Part 1: Producing Data", 2, 7, background = "#D6D3C4") %>%
pack_rows("Part 2: Organizing Data", 8, 15, background = "#D6D3C4") %>%
pack_rows("Part 3: Chance", 16, 20, background = "#D6D3C4") %>%
pack_rows("Part 4: Inference, Ethics, & Methods", 21, 30, background = "#D6D3C4") %>%
...
$**$ The final-exam window begins on *TBD*. The exam will be available during the [official final exam period](https://registrar.tamu.edu/Courses,-Registration,-Scheduling/Final-Examination-Schedules)
